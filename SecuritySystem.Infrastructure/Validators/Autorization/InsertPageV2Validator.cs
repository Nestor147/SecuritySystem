...
{
    public class InsertPageV2Validator : AbstractValidator<ResourceQueryFilter>
    {
        public InsertPageV2Validator()
        {
            RuleFor(resource => resource.Page)
                .NotEmpty()
                .WithMessage("The page is required.")
                .DependentRules(() =>
                {
                    RuleFor(resource => resource.Page)
                        .MaximumLength(50)
                        .WithMessage("The page must not exceed 50 characters.");
                });
